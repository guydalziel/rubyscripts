...
# Description: Allows users to create, list, and delete repositorios
...
def usage    
  puts "Usage: ./create-github-repo.rb <repository>
...
OptionParser.new do |opts|
  ...
  opts.on("-a", "--all", "List all (including forked repos)") do |a|
    options[:listall] = a
  end
  opts.on("-d", "--delete REPO", "Delete a repository") do |d|
    options[:delete] = d
  end
  opts.on("-l", "--list", "List GitHub Repos") do |l|
    options[:list] = l
  end
end.parse!
...
gituser = %x[git config github.user].chomp
gittoken = %x[git config github.token].chomp
...
if options[:list] == true 
  json_data = `curl -s -u \"#{gituser}:#{gittoken}\" https://api.github.com/user/repos`
  json_data = JSON.parse(json_data)
  json_data.each do |repo|
    next if options[:listall] == false && repo['fork'] == true
    puts repo['name']
  end
  exit
end
...
if options[:delete] != false
  if system("curl -u \"#{gituser}:#{gittoken}\" -X DELETE https://api.github.com/repos/#{gituser}/#{options[:delete]}")
